#!/usr/bin/env bash
#
# ops/user-suspend.sh <localpart|@user:server> — suspend an account (make it
# read-only). Reversible with ops/user-unsuspend.sh.
#
# Runs `admin users suspend <mxid>` via the admin command room. A bare localpart
...
exec python3 "${POCKET_ROOT}/scripts/lib/matrix_admin.py" users suspend "${mxid}"
